...
function startAppServer(callback) {
  ...
  const compiler = webpack({
    entry: path.resolve(__dirname, 'app_client', 'app.js'),
    module: {
      loaders: [
        {
...
function startGraphQLServer(callback) {
  ...
  clean('./app_server/schemas');
  const Schema = require('./app_server/schemas').default;
  const graphQLApp = express();

  graphQLApp.use(Logger('dev'));

  // TODO: Add authentication
  // TODO: add context user
...
const watcher = chokidar.watch('./app_server/schemas/{index}.js');
